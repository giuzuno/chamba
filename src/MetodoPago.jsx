...
export default function MetodoPago({ trabajo, onPagoExitoso, onCancelar }) {
  ...
  useEffect(() => {
    ...
    return () => {
      if (formRef.current) {
        try { formRef.current.unmount() } catch (e) { console.log('unmount form:', e) }
  ...
  function cargarSDKMercadoPago(data) {
    return new Promise((resolve) => {
      ...
      script.onerror = () => { setError('No se pudo cargar Mercado Pago. Verifica tu conexión.'); resolve() }
  ...
  function inicializarForm(data) {
    setTimeout(() => {
      try {
        const publicKey = data?.trabajadorPublicKey || import.meta.env.VITE_MP_PUBLIC_KEY
        ...
        const form = mp.cardForm({
          ...
          callbacks: {
            onFormMounted: (err) => {
              if (err) { console.error('Error montando formulario MP:', err); setError('No se pudo cargar el formulario de pago. Reintenta.'); return }
              setMpListo(true)
            },
            ...
            onFetching: (resource) => { if (resource === 'installments') setProcesando(false) }
          }
        })
  ...
  async function procesarPago(formData) {
    ...
    try {
      ...
      const { data, error: fnError } = await supabase.functions.invoke('crear-pago-mp', {
        body: { trabajoId: trabajo.id, token, issuerId, paymentMethodId, installments, email, guardarTarjeta }
      })
      if (fnError || !data?.ok) {
        ...
        if (fnError?.context) {
          ...
          } catch (e) { console.error('No se pudo parsear el body del error:', e) }
        }
  ...
  async function pagarConGuardada() {
    ...
    try {
      ...
      const { data: userData } = await supabase.auth.getUser()
      const { data, error: fnError } = await supabase.functions.invoke('crear-pago-mp', {
        body: { trabajoId: trabajo.id, token, issuerId: tarjetaGuardada.issuer.id, paymentMethodId: tarjetaGuardada.payment_method.id, installments: 1, email: userData?.user?.email || 'cliente@chamba.mx', guardarTarjeta: false }
      })
      if (fnError || !data?.ok) { setError(data?.error || 'El pago fue rechazado.'); setProcesando(false); return }
      onPagoExitoso()
    } catch {
      setError('Error al procesar el pago. Intenta de nuevo.')
      setProcesando(false)
      ...
      <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        {error && (
          <div style={{ textAlign: 'center', padding: '20px' }}>
            ...
            <button type="button" onClick={iniciarPago} style={{ padding: '12px 24px', background: '#1D9E75', color: 'white', border: 'none', borderRadius: '12px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', fontFamily: 'sans-serif' }}>Reintentar</button>
          </div>
        )}
        {!cargando && !error && (
          <>
            {tarjetaGuardada && (
              <div style={{ background: 'rgba(55,138,221,0.08)', border: '1px solid rgba(55,138,221,0.3)', borderRadius: '14px', padding: '16px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  ...
                  <button type="button" onClick={() => setUsarGuardada(!usarGuardada)} style={{ fontSize: '12px', color: usarGuardada ? '#1D9E75' : 'rgba(255,255,255,0.4)', background: 'transparent', border: 'none', cursor: 'pointer' }}>
                    {usarGuardada ? '✅ Usar esta' : 'Usar otra'}
                  </button>
                </div>
                {usarGuardada && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <input type="number" placeholder="CVV" value={cvv} onChange={e => setCvv(e.target.value)} maxLength={4} style={{ ...inputStyle, width: '120px' }} />
                    <button type="button" onClick={pagarConGuardada} disabled={procesando} style={{ width: '100%', padding: '16px', background: procesando ? 'rgba(29,158,117,0.5)' : '#1D9E75', color: 'white', border: 'none', borderRadius: '14px', fontSize: '16px', fontWeight: '600', cursor: procesando ? 'not-allowed' : 'pointer', fontFamily: 'sans-serif' }}>
            ...
            {(!tarjetaGuardada || !usarGuardada) && (
              <form id="form-checkout" onSubmit={(e) => e.preventDefault()} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                {!mpListo && <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.3)', textAlign: 'center' }}>⏳ Cargando formulario seguro de Mercado Pago...</p>}
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px', background: 'rgba(255,255,255,0.04)', borderRadius: '10px' }}>
                  <input type="checkbox" id="guardar" checked={guardarTarjeta} onChange={e => setGuardarTarjeta(e.target.checked)} style={{ width: '18px', height: '18px', accentColor: '#1D9E75', cursor: 'pointer' }} />
                  <label htmlFor="guardar" style={{ fontSize: '13px', color: 'rgba(255,255,255,0.7)', cursor: 'pointer' }}>💾 Guardar tarjeta para futuros pagos</label>
                </div>
                <button type="submit" disabled={!mpListo || procesando} style={{ width: '100%', padding: '16px', background: (!mpListo || procesando) ? 'rgba(29,158,117,0.5)' : '#1D9E75', color: 'white', border: 'none', borderRadius: '14px', fontSize: '16px', fontWeight: '600', cursor: (!mpListo || procesando) ? 'not-allowed' : 'pointer', fontFamily: 'sans-serif' }}>
                  {procesando ? 'Procesando...' : mpListo ? `💳 Pagar $${config?.totalCliente || trabajo.presupuesto} MXN` : 'Cargando...'}
                </button>
              </form>
            )}
            <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.25)', textAlign: 'center' }}>Encriptado por Mercado Pago 🔒</p>
          </>
        )}
      </div>
    </div>
