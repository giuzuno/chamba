  ...
  if (verPerfilTrabajador) {
    return (
      <PerfilPublico
        usuarioId={userId}
        rolVisto="trabajador"
        onVolver={() => setVerPerfilTrabajador(false)}
      />
    )
  }
  ...
  if (exito) {
    return (
      <div style={{
        ...
        <p style={{ color: '#1D9E75', fontSize: '32px', fontWeight: '800', marginBottom: '24px' }}>${precioActual} MXN</p>
        ...
        <button type="button" onClick={onAceptado} style={{
          ...
          fontSize: '15px', fontWeight: '500', cursor: 'pointer', fontFamily: 'sans-serif'
        }}>
          Ver mis trabajos
        </button>
      </div>
      ...
      <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.3)' }}>
              Presupuesto inicial: <span style={{ color: 'white' }}>${trabajo.presupuesto} MXN</span>
            </p>
          </div>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              {ofertas.map(oferta => (
                <div key={oferta.id} style={{ display: 'flex', justifyContent: oferta.ofertado_por === 'trabajador' ? 'flex-end' : 'flex-start' }}>
                  <div style={{
                    background: oferta.ofertado_por === 'trabajador' ? 'rgba(29,158,117,0.15)' : 'rgba(55,138,221,0.15)',
                    border: `0.5px solid ${oferta.ofertado_por === 'trabajador' ? 'rgba(29,158,117,0.3)' : 'rgba(55,138,221,0.3)'}`,
                    ...
                    <p style={{ fontSize: '18px', fontWeight: '700', color: oferta.ofertado_por === 'trabajador' ? '#1D9E75' : '#378ADD' }}>
                      ${oferta.monto} MXN
                    </p>
                  </div>
                </div>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ display: 'flex', gap: '4px', justifyContent: 'flex-end' }}>
              {Array.from({ length: MAX_RONDAS }).map((_, i) => (
                <div key={i} style={{ width: '10px', height: '10px', borderRadius: '50%', background: i < rondasRestantes ? '#1D9E75' : 'rgba(255,255,255,0.15)' }} />
              ))}
            </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '8px' }}>
              <input type="range" min="100" max="5000" step="50"
                value={nuevaOferta} onChange={e => setNuevaOferta(Number(e.target.value))}
            ...
            {nuevaOferta > precioActual && (
              <div style={{ background: 'rgba(186,117,23,0.1)', border: '0.5px solid rgba(186,117,23,0.3)', borderRadius: '10px', padding: '8px 12px', fontSize: '12px', color: '#E8A030' }}>
                ↑ Estás pidiendo ${nuevaOferta - precioActual} MXN más
              </div>
            )}
            {nuevaOferta < precioActual && (
              <div style={{ background: 'rgba(29,158,117,0.08)', border: '0.5px solid rgba(29,158,117,0.2)', borderRadius: '10px', padding: '8px 12px', fontSize: '12px', color: '#5DCAA5' }}>
        ...
        {rondasRestantes > 0 && nuevaOferta !== precioActual && (
          <button type="button" onClick={hacerContraoferta} disabled={loading} style={{
            width: '100%', padding: '14px', background: 'transparent', color: '#1D9E75',
            border: '1.5px solid #1D9E75', borderRadius: '14px', fontSize: '15px',
        ...
        <button type="button" onClick={onVolver} style={{
          width: '100%', padding: '14px', background: 'transparent', color: 'rgba(255,255,255,0.3)',
          border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '14px', fontSize: '14px',
